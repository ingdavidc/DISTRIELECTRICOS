    ...
    try {
      ...
      if (res.success) {
        if (amountToPay < currentBalance) {
          toast.success(`Abono exitoso. Saldo pendiente: $${(currentBalance - amountToPay).toLocaleString()}`, { id: tid });
        } else {
                  ...
                  <div>
                    ...
                    {order.amountPaid > 0 && (
                       <div style={{ fontSize: "0.85rem", color: "var(--color-secondary)", marginTop: "0.25rem", fontWeight: 600 }}>
                         Abonado: ${order.amountPaid.toLocaleString()}
                       </div>
                    )}
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: "0.8rem", color: "var(--color-text-muted)", fontWeight: 600 }}>SALDO</div>
                    <div style={{ fontSize: "1.5rem", fontWeight: 700, color: "var(--color-primary)" }}>${balance.toLocaleString()}</div>
          ...
          <>
                  ...
                  {editableItems.map(item => (
                    <div key={item.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingBottom: "0.75rem", borderBottom: "1px dashed var(--color-border)" }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: "0.85rem", color: "var(--color-text-muted)" }}>${item.unitPrice.toLocaleString()} c/u</div>
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: "1.5rem" }}>
                        ...
                        <div style={{ fontWeight: 600, width: "80px", textAlign: "right" }}>${(item.quantity * item.unitPrice).toLocaleString()}</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div style={{ padding: "1.5rem" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "1.5rem" }}>
                  ...
                  {amountToPay < currentBalance && (
                    <span className="badge badge-warning">Quedará saldo de ${(currentBalance - amountToPay).toLocaleString()}</span>
                  )}
                </div>
            ...
            <div style={{ padding: "1.5rem", borderTop: "1px solid var(--color-border)", background: "var(--color-background)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
                ...
                <span style={{ fontSize: "1.5rem", fontWeight: 700, color: "var(--color-text-main)" }}>${currentBalance.toLocaleString()}</span>
              </div>
              <button 
              ...
              >
                <CheckCircle size={24} />
                {isProcessing ? "Procesando..." : `RECAUDAR $${amountToPay.toLocaleString()} Y GUARDAR`}
              </button>
            </div>
          </>
        ) : (
